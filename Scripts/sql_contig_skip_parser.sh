#!/bin/bash\
#
# This script takes a list of bins and grabs all contig information from the .faa proteins used for the MySQL database. 
...
do {
	...
	cat $file | egrep -v "^#" | awk 'NR==1 {print ; next} {printf /^>/ ? "\n"$0"\n" : $1} END {printf "\n"}' | awk -v file_loc=$file -v bin_ext=$2 'BEGIN{OFS="\t"}
	 ...
	 print c[length(c)], c[length(c)], "\\N", tot*3, "\\N", "\\N";
	 }'
}
done | egrep -v "^/" > $3
